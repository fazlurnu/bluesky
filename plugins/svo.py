...
class SVO(ConflictResolution):
    ...
    def do_maneuver(self, gs_own, hdg_own, gs_int, hdg_int):      
    ...
    def SVO(self, ownship, intruder, conf, qdr, dist, tLOS, idx1, idx2):
        ...
        if((tp_1 != None) & (tp_2 != None)):
            ...
            # method = 0: opt, 1: spd change, 2: hdg change, 4: svo
            if(method == 4):
                cp_1 = nearest_points(vo_line_1, ownship_velocity)[0]
                cp_2 = nearest_points(vo_line_2, ownship_velocity)[0]

                curr_hdg = np.degrees(np.arctan2(ownship.gseast[idx1], ownship.gsnorth[idx1]))
                ...
                do_maneuver = self.do_maneuver(gs_own, hdg_own, gs_int, hdg_int)
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        for ((ac1, ac2), qdr, dist, tcpa, tLOS) in zip(conf.confpairs, conf.qdr, conf.dist, conf.tcpa, conf.tLOS):
            ...
            if idx1 >-1 and idx2 > -1:
                dv_mvp, tsolV = self.SVO(ownship, intruder, conf, qdr, dist, tLOS, idx1, idx2)
